    ...
    try {
        ...
        const productDocs = await Promise.all(
            products.map(async (product) => {
                const brand = await SecondaryBrand.findOne({
                    _id: product.brandId,
                }).lean();
                ...
                await Promise.all([
                    ...
                    SecondaryProduct.updateOne(
                        { _id: new ObjectId(savedProduct._id) },
                        { HistoryID: secondaryResult?.[0]?._id }
                    ),
                ]);
...
const deleteSelectedProduct = async (req, res) => {
    const product = await SecondaryProduct.findOne({
        _id: req.params.id,
    }).lean();
    const deleteProduct = await SecondaryProduct.deleteOne({
        ...
        await PrimaryProduct.findByIdAndUpdate(req.params.id, {
            isActive: false,
        }).catch(() => {
    ...
    const deletePurchaseProduct = await SecondaryPurchase.deleteOne({
        ...
        await PrimaryPurchase.findByIdAndUpdate(
            { ProductID: req.params.id },
            { isActive: false }
        ).catch(() => {
...
const searchProduct = async (req, res) => {
    try {
        ...
        if (req?.headers?.role === ROLES.HIDE_MASTER_SUPER_ADMIN)
            findAllProducts = await primaryModel.aggregate(pipeline);
        else findAllProducts = await secondaryModel.aggregate(pipeline); // -1 for descending;
        res.json(findAllProducts);
    } catch (error) {
...
const searchProductByWarehouse = async (req, res) => {
    try {
        ...
        let findAllProducts;
        if (req?.headers?.role === ROLES.HIDE_MASTER_SUPER_ADMIN)
            findAllProducts = await PrimaryAvailableStock.aggregate([
                {
                    $lookup: {
                        from: 'product',
                        localField: 'productID',
            ...
            findAllProducts = await SecondaryAvailableStock.aggregate([
                {
                    $match: {
                        warehouseID: new ObjectId(searchTerm),
                    },
    ...
    try {
        ...
        if (req?.headers?.role === ROLES.HIDE_MASTER_SUPER_ADMIN) {
            ...
            totalItemInWarehouse =
                await PrimaryAvailableStock.find(filter).count();
        } else {
            totalProductCounts = await SecondaryProduct.find().count();
            totalItemInWarehouse =
                await SecondaryAvailableStock.find(filter).count();
        }
