...
const getAllHistory = async (req, res) => {
    ...
    else findAllHistory = await SecondaryHistory.aggregate(pipeline);
...
const deleteSelectedHistory = async (req, res) => {
    ...
    if (
        ...
        !findHistory?.purchaseID &&
        !findHistory?.saleID &&
        !findHistory?.writeOffID
    ) {
        const deleteProduct = await SecondaryProduct.deleteOne({
            _id: findHistory?.productID,
        }).then(async (result) => {
            await PrimaryProduct.findByIdAndUpdate(findHistory?.productID, {
                isActive: false,
            }).catch(() => {
    ...
    if (findHistory?.purchaseID) {
        const [secondaryPurchaseInfos, primaryPurchaseInfos] =
            await Promise.all([
        ...
        const deletePurchaseProduct = await SecondaryPurchase.deleteOne({
            ...
            const data = await PrimaryPurchase.findOne({
                _id: findHistory?.purchaseID,
            });
            console.log('data Primary purchase', data);
            await PrimaryPurchase.findByIdAndUpdate(findHistory?.purchaseID, {
                isActive: false,
            }).catch((e) => {
        ...
        const secondaryProductData = await SecondaryProduct.findOne({
            _id: findHistory?.productID,
        });
        if (secondaryProductData?.stock) {
            console.log(
                ...
                secondaryProductData,
                secondaryPurchaseInfos
            );
            let secondaryUpdatedStock =
                Number(secondaryProductData.stock) -
                Number(secondaryPurchaseInfos?.QuantityPurchased || 0);
        ...
        const secondaryAvailableData = await SecondaryAvailableStock.findOne({
            productID: findHistory?.productID,
            warehouseID: secondaryPurchaseInfos?.warehouseID,
        });
        if (secondaryAvailableData?.stock) {
            ...
            let secondaryAvailableStock =
                Number(secondaryAvailableData.stock) -
                Number(secondaryPurchaseInfos?.QuantityPurchased || 0);
        ...
        // Primary Product
        const primaryProductData = await PrimaryProduct.findOne({
            _id: findHistory?.productID,
        });
        if (primaryProductData?.stock) {
            let primaryUpdatedStock =
                Number(primaryProductData.stock) -
                Number(primaryPurchaseInfos?.QuantityPurchased);
        ...
        const primaryAvailableData = await PrimaryAvailableStock.findOne({
            productID: findHistory?.productID,
            warehouseID: primaryPurchaseInfos?.warehouseID,
        });
        console.log(
            'secondaryProductData',
            secondaryProductData,
            primaryPurchaseInfos
        );
        if (primaryAvailableData?.stock) {
            let primaryAvailableStock =
                Number(primaryAvailableData.stock) -
                Number(primaryPurchaseInfos?.QuantityPurchased || 0);
    ...
    if (findHistory?.saleID) {
        ...
        const deleteSaleProduct = await SecondarySales.deleteOne({
            ...
            await PrimarySales.findByIdAndUpdate(findHistory?.saleID, {
                isActive: false,
            }).catch(() => {
        ...
        if (secondaryProductData?.stock) {
            let secondaryUpdatedStock =
                Number(secondaryProductData.stock) +
                Number(secondaryPurchaseInfos?.StockSold || 0);
        ...
        if (primaryProductData?.stock) {
            let primaryUpdatedStock =
                Number(primaryProductData.stock) +
                Number(primaryPurchaseInfos?.StockSold);
        ...
        if (primaryAvailableData?.stock) {
            let primaryAvailableStock =
                Number(primaryAvailableData.stock) +
                Number(primaryPurchaseInfos?.StockSold || 0);
    ...
    if (findHistory?.writeOffID) {
        ...
        const deleteSaleProduct = await SecondaryWriteOff.deleteOne({
            ...
            await PrimaryWriteOff.findByIdAndUpdate(findHistory?.writeOffID, {
                isActive: false,
            }).catch(() => {
                console.log('Primary sales error');
            });
    ...
    const deleteHistory = await SecondaryHistory.findByIdAndUpdate(
        req.params.id,
        { isActive: false }
    ).then(async () => {
        await PrimaryHistory.findByIdAndUpdate(req.params.id, {
            isActive: false,
        }).catch(() => {
            console.log('Primary product error');
        });
...
const updateSelectedHistory = async (req, res) => {
    try {
        ...
        await PrimaryHistory.findByIdAndUpdate(
            { _id: req.body.productID },
            {
                name: req.body.name,
                manufacturer: req.body.manufacturer,
...
const addHistoryData = async (
    ...
    try {
        ...
        if (
            role === ROLES.HIDE_MASTER_SUPER_ADMIN &&
            method &&
            METHODS.ADD !== method
        ) {
        ...
        if (method === METHODS.ADD) {
            if (type === HISTORY_TYPE.DELETE) {
                secondaryResult = await SecondaryHistory.insertMany([
                    updatedSecondaryPayload,
                ]).catch((err) => console.log('Err', err));
                if (role === ROLES.HIDE_MASTER_SUPER_ADMIN) {
                    primaryResult = await PrimaryHistory.insertMany([
                        { ...data, _id: secondaryResult?.[0]?._id },
                    ]).catch((err) => console.log('Err', err));
                    if (type === HISTORY_TYPE.DELETE)
                        await SecondaryHistory.deleteMany({
                            productID: secondaryResult?.[0]?.productID,
                        });
                }
            } else {
                secondaryResult = await SecondaryHistory.insertMany([
                ...
                primaryResult = await PrimaryHistory.insertMany([
                    {
                        ...updatedSecondaryPayload,
                        _id: secondaryResult?.[0]?._id,
                    },
                ]).catch((err) => console.log('Err', err));
            }
        } else if (method === METHODS.UPDATE) {
            if (type === HISTORY_TYPE.DELETE) {
                secondaryResult = await SecondaryHistory.updateMany(
                    { _id: data?.historyID },
                    [updatedSecondaryPayload]
                ).catch((err) => console.log('Err', err));
                if (role === ROLES.HIDE_MASTER_SUPER_ADMIN) {
                    primaryResult = await PrimaryHistory.updateMany(
                        { _id: data?.historyID },
                        [{ ...data, _id: secondaryResult?.[0]?._id }]
                    ).catch((err) => console.log('Err', err));
                    if (type === HISTORY_TYPE.DELETE)
