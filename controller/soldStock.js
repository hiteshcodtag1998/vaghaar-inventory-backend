...
const soldStock = async (
    ...
    // Updating sold stock
    try {
        const myProductData = await SecondaryProduct.findOne({
            _id: productID,
        });
        let myUpdatedStock = myProductData?.stock
            ? myProductData?.stock - stockSoldData
            : 0;
        if (isUpdate) {
            const pendingStock = Number(beforeStock) - Number(stockSoldData);
            myUpdatedStock =
                myProductData?.stock || myProductData?.stock === 0
                    ? Number(myProductData?.stock) + pendingStock
                    : 0;
        ...
        let primaryUpdatedStock = primaryProductData?.stock
            ? primaryProductData?.stock - stockSoldData
            : 0;

        if (isUpdate) {
            const pendingStock = Number(beforeStock) - Number(stockSoldData);
            primaryUpdatedStock =
                primaryProductData?.stock || primaryProductData?.stock === 0
                    ? Number(primaryProductData?.stock) + pendingStock
                    : 0;
            // primaryUpdatedStock = primaryProductData?.stock ? (primaryProductData?.stock + primaryProductData?.stock - stockSoldData) : 0;
        }
