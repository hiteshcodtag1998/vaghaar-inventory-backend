        ...
        const saleDocs = await Promise.all(
            sales.map(async (sale) => {
                ...
                if (
                    ...
                    existsAvailableStock.stock < sale.stockSold
                ) {
                    throw new Error(
                ...
                if (!purchases || purchases.length === 0) {
                    throw new Error(
                        'No purchases available to fulfill the sale'
                    );
                }
                ...
                for (const purchase of purchases) {
                    ...
                    const effectiveRemainingStock =
                        purchase.remainingStock ?? purchase.QuantityPurchased;
                    const deductStock = Math.min(
                        remainingStock,
                        effectiveRemainingStock
                    );
                    ...
                    if (deductStock > 0) {
                        ...
                        await SecondaryPurchase.findByIdAndUpdate(
                            purchase._id,
                            {
                                ...
                                $set: {
                                    isUsed:
                                        effectiveRemainingStock - deductStock <=
                                        0,
                                },
                            }
                        );
                ...
                if (remainingStock > 0) {
                    throw new Error(
                        'Insufficient purchase stock to fulfill the sale'
                    );
                }
...
const getSalesData = async (req, res) => {
    ...
    else findAllSalesData = await SecondarySales.aggregate(aggregationPiepline); // -1 for descending;
...
const getTotalSalesAmount = async (req, res) => {
    ...
    else salesData = await SecondarySales.find().lean();
...
const getMonthlySales = async (req, res) => {
    try {
        ...
        else sales = await SecondarySales.find().lean();
    ...
    try {
        ...
        if (stockDifference > 0) {
            ...
            for (const purchase of purchases) {
                ...
                if (purchase.remainingStock > 0) {
                    ...
                    await SecondaryPurchase.findByIdAndUpdate(purchase._id, {
                        $inc: { remainingStock: -deductStock },
                        $set: {
                            isUsed: purchase.remainingStock - deductStock === 0,
                        },
                    });
            ...
            if (remainingToProcess > 0) {
                throw new Error(
                    'Insufficient stock in linked purchases for the update.'
                );
            }
            ...
            for (const purchase of purchases) {
                ...
                const updatedRemainingStock =
                    purchase.remainingStock + remainingToProcess;
                ...
                remainingToProcess -=
                    newRemainingStock - purchase.remainingStock;
            }
        }
        ...
        await soldStock(
            req.body.productID,
            req.body.stockSold - existingSale.StockSold
        );
