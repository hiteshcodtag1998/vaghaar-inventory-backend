...
const {
    PrimaryAvailableStock,
    SecondaryAvailableStock,
} = require('../models/availableStock');
...
} = require('../utils/templates/invoice-bill-multiple-item');
    ...
    try {
        ...
        const purchaseDocs = await Promise.all(
            prchases.map(async (product) => {
                ...
                const requestby = req?.headers?.requestby
                    ? new ObjectId(req.headers.requestby)
                    : '';
                // Start History Data
                const productInfo = await SecondaryProduct.findOne({
                ...
                const { primaryResult, secondaryResult } = await addHistoryData(
                    historyPayload,
                    req?.headers?.role,
                    null,
                    METHODS.ADD
                ...
                purchaseProduct = {
                    ...purchaseProduct._doc,
                    HistoryID: secondaryResult?.[0]?._id,
                };
                await Promise.all([
                    PrimaryPurchase.insertMany([purchaseProduct]),
                    SecondaryPurchase.updateOne(
                        { _id: new ObjectId(purchaseProduct._id) },
                        { HistoryID: secondaryResult?.[0]?._id }
                    ),
                ]);
...
const getAllPurchaseData = async (req, res) => {
    ...
    if (req?.headers?.role === ROLES.HIDE_MASTER_SUPER_ADMIN)
        findAllPurchaseData =
            await PrimaryPurchase.aggregate(aggregationPiepline);
    else
        findAllPurchaseData =
            await SecondaryPurchase.aggregate(aggregationPiepline); // -1 for descending;
    res.json(findAllPurchaseData);
};
...
const getPurchaseData = async (req, res) => {
    ...
    if (req?.headers?.role === ROLES.HIDE_MASTER_SUPER_ADMIN)
        findPurchaseData = await PrimaryPurchase.findById(req?.params?.id)
            .sort({ createdAt: -1 })
            .lean();
    else
        findPurchaseData = await SecondaryPurchase.findById(req?.params?.id)
            .sort({ createdAt: -1 })
            .lean(); // -1 for descending;
...
const updateSelectedPurchaase = async (req, res) => {
    try {
        ...
        if (![ROLES.HIDE_MASTER_SUPER_ADMIN].includes(req?.headers?.role)) {
            ...
            updatedResult = await PrimaryPurchase.findByIdAndUpdate(
                { _id: req.body.purchaseID },
                {
                    QuantityPurchased: req.body.quantityPurchased,
                    PurchaseDate: req.body.purchaseDate,
            ...
            const requestby = req?.headers?.requestby
                ...
                : '';
            // Start History Data
            const productInfo = await PrimaryProduct.findOne({
                _id: updatedResult.ProductID,
            });
            const historyPayload = {
                productID: updatedResult.ProductID,
...
const purchaseMultileItemsPdfDownload = async (req, res) => {
    try {
        ...
        if (req.body?.[0]?.warehouseID) {
            const warehouseInfos = await SecondaryWarehouse.findOne({
                _id: new ObjectId(req.body?.[0]?.warehouseID),
            }).lean();
        ...
        await Promise.all(
            prchases.map(async (purchase) => {
                ...
                const productInfos =
                    await SecondaryProduct.aggregate(aggregationPiepline);
                if (productInfos?.length > 0) {
                    payload.productName.push(productInfos[0]?.name || '');
                    payload.qty.push(purchase.quantityPurchased || '');
                    payload.brandName.push(
                        productInfos[0]?.BrandID?.name || ''
                    );
                }
            })
        );
...
const getPurchaseDataByProductId = async (req, res) => {
    ...
    if (req?.headers?.role === ROLES.HIDE_MASTER_SUPER_ADMIN)
        findPurchaseData = await PrimaryPurchase.find({
            ProductID: req?.params?.productId,
        })
            .sort({ createdAt: -1 })
            .lean();
    else
        findPurchaseData = await SecondaryPurchase.find({
            ProductID: req?.params?.productId,
        })
            .sort({ createdAt: -1 })
            .lean(); // -1 for descending;
...
const deleteSelectedPurchase = async (req, res) => {
    try {
        ...
        const purchaseInfo = await SecondaryPurchase.findOne({
            _id: req.params.id,
        }).lean();
        ...
        const deletePurchase = await SecondaryPurchase.deleteOne({
            ...
            await PrimaryPurchase.findByIdAndUpdate(req.params.id, {
                isActive: false,
            }).catch(() => {
        ...
        needToRemoveItem = await deleteOrUpdateSaleByPurchase(
            purchaseInfo,
            needToRemoveItem
        );

        needToRemoveItem = await deleteOrUpdateWriteOffByPurchase(
            purchaseInfo,
            needToRemoveItem
        );
        ...
        if (remainingStockToRedistribute > 0) {
            await adjustRemainingStock(
                purchaseInfo,
                remainingStockToRedistribute
            );
        }
    ...
    try {
        ...
        for (const sale of sales) {
            ...
            if (
                ...
                linkedPurchases[0].toString() === purchaseId?.toString()
            ) {
                ...
                console.log(
                    `Sale ${sale._id} deleted as it was only linked to purchase ${purchaseId}`
                );
            } else {
...
const deleteOrUpdateWriteOffByPurchase = async (
    purchaseInfo,
    needToRemoveItem
) => {
    try {
        ...
        for (const writeOff of writeOffs) {
            ...
            if (
                ...
                console.log(
                    `Write-off ${writeOff._id} deleted as it was only linked to purchase ${purchaseId}`
                );
            } else {
                // Case 2: Multiple linked purchases - decrease the stock
...
const adjustRemainingStock = async (
    deletedPurchase,
    remainingStockToRedistribute
) => {
    try {
        ...
        for (const purchase of remainingPurchases) {
            ...
            const availableSpace =
                purchase.QuantityPurchased - purchase.remainingStock;
            const stockToAdd = Math.min(
                remainingStockToRedistribute,
                availableSpace
            );
            ...
            await SecondaryPurchase.findByIdAndUpdate(purchase._id, {
                ...
                $set: {
                    isUsed:
                        purchase.remainingStock + stockToAdd <
        ...
        if (remainingStockToRedistribute > 0) {
            console.log(
                `Unallocated stock: ${remainingStockToRedistribute} could not be redistributed.`
            );
        }
    } catch (error) {
...
const pullStockFromAvailableStock = async (purchaseInfo) => {
    try {
        ...
        console.log(
            `Removed ${QuantityPurchased} stock for product ${productId} from both Primary and Secondary collections`
        );

        purchaseStock(productId, QuantityPurchased, true);
    } catch (error) {
        console.log(
            'Error while pulling stock and updating product stocks',
            error
        );
    }
};
...
module.exports = {
    ...
    purchasePdfDownload,
    purchaseMultileItemsPdfDownload,
    updateSelectedPurchaase,
    getPurchaseDataByProductId,
