const {
    PrimaryTransferStock,
    SecondaryTransferStock,
} = require('../models/transferStock');
const {
...
const addTransferStock = async (req, res) => {
    try {
        ...
        if (
            ...
            existsAvailableStock?.stock < product.quantityPurchased
        ) {
        ...
        const stocksRes = await SecondaryAvailableStock.insertMany([
            availableStockPayload,
        ]);
        await PrimaryAvailableStock.insertMany(stocksRes);

        await SecondaryAvailableStock.findOneAndUpdate(
            new ObjectId(existsAvailableStock?._id),
            { $inc: { stock: -product.quantityPurchased } }
        );
        await PrimaryAvailableStock.findOneAndUpdate(
            new ObjectId(existsAvailableStock?._id),
            { $inc: { stock: -product.quantityPurchased } }
        );
        ...
        const transferData = await addTransferStockDetails.save();
        // .then(async (result) => {
        ...
        // End History Data
        await PrimaryTransferStock.insertMany([transferData]).catch((err) =>
            console.log('Err', err)
        );
...
const getTransferStockData = async (req, res) => {
    ...
    if (req?.headers?.role === ROLES.HIDE_MASTER_SUPER_ADMIN)
        findAllWriteOffData =
            await PrimaryTransferStock.aggregate(aggregationPiepline);
    else
        findAllWriteOffData =
            await SecondaryTransferStock.aggregate(aggregationPiepline); // -1 for descending;
