...
const addBrand = async (req, res) => {
    try {
        const brandExist = await SecondaryBrand.findOne({
            name: req.body.name,
        });
        ...
        addBrand
            .save()
            .then(async (result) => {
                await PrimaryBrand.insertMany([result]).catch((err) =>
                    console.log('Err', err)
                );
...
const getAllBrands = async (req, res) => {
    ...
    else findAllBrands = await SecondaryBrand.find().sort({ _id: -1 }); // -1 for descending;
